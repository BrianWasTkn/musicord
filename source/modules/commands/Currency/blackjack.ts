...
export default class Currency extends Command {
	...
	async exec(
		...
		const args = ((bet: number) => {
			...
			switch (true) {
				...
				case data.pocket >= MAX_POCKET:
					return { state, m: `You're too rich (${MAX_POCKET.toLocaleString()}) to play!` };
				...
				case bet < MIN_BET:
					return { state, m: `You can't play lower than **${MIN_BET}** coins sorry` };
				case bet > MAX_BET:
					return { state, m: `You can't play higher than **${MAX_BET.toLocaleString()}** coins sorry` };
				default:
		...
		const gambed = async (final?: boolean): Promise<MessageOptions> => {
			...
			if (status.constructor === Object) {
				...
				// Win
				if (status.result) {
					winnings = Math.ceil(bet * (Math.random() + (0.2 + extraWngs))); // "Base Multi"
		...
		const dealersTurn = (end?: boolean): Promise<MessageOptions> => {
			if (addCards('user') > 21) {
				return gambed();
			}
			const thoughts = [];
