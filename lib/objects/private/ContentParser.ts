import { ArgumentOptions, Constants } from 'discord-akairo';
...
class Parser {
	...
	results: {
		all: any[];
		phrases: { type: 'Phrase' | string, value: any, raw: any }[];
		flags: { type: 'Flag' | string, key: any, raw: any }[];
		optionFlags: { type: 'OptionFlag' | string, value: any, key: any, raw: any }[];
	};

    constructor(tokens: any[], { separated }: Partial<{ separated: boolean; }>) {
        this.tokens = tokens;
...
export class ContentParser {
    ...
    static getFlags(args: ArgumentOptions[]) {
        const res: { flagWords: any[], optionFlagWords: any[] } = {
            flagWords: [],
            optionFlagWords: []
        };
