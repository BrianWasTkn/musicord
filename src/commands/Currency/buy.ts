...
export default class Currency extends Command {
  ...
  async exec(msg: Message, args: {
    ...
    const { paid, amount: amt, data: dat, item: i,} = await Items.buy(amount, msg.author.id, item.id);
    const embed = new Embed()
      .setDescription(`Succesfully purchased \`${amt.toLocaleString()}\` **${i.name}**${amt > 1 ? 's' : ''} and paid \`${paid.toLocaleString()}\` coins.`)
