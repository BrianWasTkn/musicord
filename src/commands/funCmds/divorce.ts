...
export default class Fun extends Command {
  ...
  async exec(ctx: Context): Promise<MessageOptions> {
    ...
    const husOrWif = (await this.client.users.fetch(
      me.marriage.id
    )) as UserPlus;
    await ctx.send({
      content: `Are you sure you wanna have a divorce with ${husOrWif.username}? Type \`y\` or \`n\` in 30 seconds.`,
    });
    const filt = (m) => m.author.id === ctx.author.id;
    const resp = (
      await ctx.channel.awaitMessages(filt, { max: 1, time: 3e4 })
    ).first();
    ...
    return {
      replyTo: ctx.id,
      content: `Divorce against ${husOrWif.tag} successful.`,
    };
  }
}
