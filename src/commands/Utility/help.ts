...
export default class Utility extends Command {
    ...
    constructor() {
        super('help', {
            ...
            channel: 'guild',
            description:
                'Fetches all public commands of this bot.',
            category: 'Utility',
            args: [
                {
                    id: 'query',
    ...
    private fieldifyCmd(c: Command & {
        ...
        return (new MessageEmbed()
            ...
            .addField('CD Ratelimit', c.ratelimit || 1, true)
        ).fields
    }
    ...
    public async exec(_: Message, args: Help): Promise<Message> {
        ...
        if (cmd && !cat) {
            ...
            embed.setTitle(`${this.handler.prefix[0]} ${cmd.aliases[0]} info`)
            .setFooter(`Requested by: ${_.author.tag}`, _.author.avatarURL({ dynamic: true }))
            .addFields(fields).setColor('ORANGE')
            return _.channel.send({ embed })
        } else if (cat) {
            embed.setTitle(`${cat.array()[0].categoryID} Commands`)
            .setDescription(`\`${cat.array().map(c => c.aliases[0]).join('`, `')}\``)
            .setColor('ORANGE').setFooter(this.client.user.username, this.client.user.avatarURL())
        } else {
