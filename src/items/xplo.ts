...
export default class PowerUp extends Item {
  ...
  async use(ctx: Context): Promise<MessageOptions> {
    ...
    if (odds >= 30) {
      const mods = this.client.handlers.item.modules.array().filter(i => i.categoryID !== 'Boxes').filter((i) => i.cost <= 5e5);
      ...
      for (let e = 0; e < randomNumber(1, 3); e++) {
        ...
        const item = randomInArray(mods.filter(i => i.categoryID !== 'Boxes').filter(filter));
