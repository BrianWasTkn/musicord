import { CollectorFilter, Collection, TextChannel, Message } from 'discord.js';
...
async function handleDonation(
	...
	try {
		...
		try {
			const questions: { [q: string]: string } = qObj[type];
			await dm.send(
			...
			const fcol = (
				await dm.awaitMessages(filter as (
					(m: Message) => boolean
				), { max: 1, time: 60000 })
			).first();
			...
			let index: number = 0;
			async function collect(question: string): Promise<boolean | Function> {
